...
const requestBookByTitle = (title, callback) => {
  request(`https://austin.bibliocommons.com/search?custom_edit=false&custom_query=title%3A%28${encodeURI(title)}%29+formatcode%3A%28BK+%29&display_quantity=100`, function (error, response, html) {
    if (!error) {
      const $ = cheerio.load(html);
      
      const first = $(".list_item_outer").first();
      const url = first.find("span.title a").attr("href");
      
      if (url) {
        request(`https://austin.bibliocommons.com${url}`, function (error, response, html) {
          if (!error) {
            const $ = cheerio.load(html);
            const isbn = $(".itemDetail").attr("data-isbn");
            if (isbn) {
              requestBook(isbn, callback);
            } else {
              callback({});
            }
          } else {
